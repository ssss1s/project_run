...
def upload_file(request):
    ...
    try:
        ...
        # Заранее получаем все данные для проверки
        existing_items = CollectibleItem.objects.values_list('uid', 'name')
        existing_uids = {uid for uid, name in existing_items}
        existing_names_uids = {(name, uid) for uid, name in existing_items}
        ...
        for row_idx, row in enumerate(ws.iter_rows(values_only=True), start=1):
            ...
            try:
                ...
                # 4. Проверка на существование в БД (по UID и паре Name+UID)
                if uid in existing_uids or (name, uid) in existing_names_uids:
